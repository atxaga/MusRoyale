...
class PrivateMatchSetupActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_duos)

        val btnInviteFriend: MaterialButton = findViewById(R.id.btnInviteFriend)
        val toggleGroupReyes: MaterialButtonToggleGroup = findViewById(R.id.toggleGroupReyes)
        val btnPlay: Button = findViewById(R.id.btnPlay)
        val etCustomBet: TextInputEditText = findViewById(R.id.etCustomBet)
        ...
        // Botón de invitar amigo
        btnInviteFriend.setOnClickListener {
            Snackbar.make(btnInviteFriend, "Funcionalidad de invitar amigo próximamente", Snackbar.LENGTH_SHORT).show()
            // TODO: Implementar lógica para invitar amigos
        }

        betButtons.forEach { button ->
            button.setOnClickListener {
                selectBetButton(button)
                etCustomBet.text?.clear()
            }
        }

        // Cuando el usuario escribe una cantidad personalizada, deseleccionar los botones
        etCustomBet.addTextChangedListener(object : TextWatcher {
            override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {}
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                if (!s.isNullOrEmpty()) {
                    deselectAllBetButtons()
                }
            }
        ...
        btnPlay.setOnClickListener {
            ...
            // Priorizar apuesta personalizada si existe
            val selectedBet = if (!etCustomBet.text.isNullOrEmpty()) {
                "${etCustomBet.text} €"
            } else {
                selectedBetButton?.text?.toString() ?: getString(R.string.match_bet_250)
            }

            val message = "Partida privada · $selectedMode · Apuesta $selectedBet"
    ...
    private fun deselectAllBetButtons() {
        selectedBetButton?.let {
            it.backgroundTintList = ColorStateList.valueOf(ContextCompat.getColor(this, R.color.toggle_bg_unselected))
            it.strokeColor = ColorStateList.valueOf(ContextCompat.getColor(this, R.color.toggle_stroke_unselected))
            it.setTextColor(ContextCompat.getColor(this, R.color.white))
